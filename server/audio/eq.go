...
// eqBand holds parameters and the filter for a single EQ band.
type eqBand struct {
	frequency float64
	gain      float64
	q         float64
...
func (eq *EQ) Process(samples []float32) []float32 {
	eq.mu.Lock()
	defer eq.mu.Unlock()
	...
	for i := 0; i < 3; i++ {
		...
		filter := &eq.bands[i].filter
		for j := range samples {
			samples[j] = float32(filter.Process(float64(samples[j])))
		}
	}
	return samples
}
