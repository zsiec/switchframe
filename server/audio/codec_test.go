...
// mockDecoder implements Decoder for testing.
...
// mockEncoder implements Encoder for testing.
...
func TestCodecInterfaces(t *testing.T) {
	var dec Decoder = &mockDecoder{samples: []float32{0.5, -0.5}}
	samples, err := dec.Decode([]byte{0x01})
	require.NoError(t, err)
	require.Equal(t, []float32{0.5, -0.5}, samples)

	var enc Encoder = &mockEncoder{data: []byte{0xFF}}
