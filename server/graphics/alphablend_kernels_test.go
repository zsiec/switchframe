...
func TestAlphaBlendRGBARowY_CrossValidation(t *testing.T) {
	...
	// Reference: use the original float64 AlphaBlendRGBA on a 1-row "frame"
	yuvRef := make([]byte, width+width/2) // Y + Cb + Cr for 1 row (height=1 needs special handling)
	// For height=1, we need width*1 + (width/2)*(1/2) which doesn't work well.
	// Instead, use a 2-row frame and compare just the first row.
	height := 2
