...
demo: build-server node_modules_check
	...
	@trap 'kill 0' EXIT; \
		...
		if [ -d test/clips ]; then \
			./bin/switchframe --demo --demo-video test/clips --format 1080p59.94 --frame-sync --frc-quality mcfi --decode-all-sources --scte35 --captions $$CERT_FLAGS & \
		else \
			./bin/switchframe --demo --format 1080p59.94 --frame-sync --frc-quality mcfi --decode-all-sources --scte35 --captions $$CERT_FLAGS & \
		fi; \
		cd ui && npm run dev & \
		wait
