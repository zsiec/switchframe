...
describe('sortedSourceKeys', () => {
	it('sorts by position ascending', () => {
		const sources: Record<string, SourceInfo> = {
			'cam-wide': { key: 'cam-wide', type: 'demo', status: 'healthy', position: 3 },
			'cam-1': { key: 'cam-1', type: 'demo', status: 'healthy', position: 1 },
			'cam-2': { key: 'cam-2', type: 'demo', status: 'healthy', position: 2 },
		};
	...
	it('falls back to alphabetical for equal positions', () => {
		const sources: Record<string, SourceInfo> = {
			'bravo': { key: 'bravo', type: 'demo', status: 'healthy', position: 1 },
			'alpha': { key: 'alpha', type: 'demo', status: 'healthy', position: 1 },
		};
	...
	it('handles missing position (defaults to 0)', () => {
		const sources: Record<string, SourceInfo> = {
			'cam-1': { key: 'cam-1', type: 'demo', status: 'healthy' } as SourceInfo,
			'cam-2': { key: 'cam-2', type: 'demo', status: 'healthy', position: 1 },
		};
